...
function ApiKeysTab() {
  ...
  async function handleSave() {
    ...
    setSaved(false);
    setSaveError(null);
    const SECRET_KEYS = ["geminiApiKey", "googlePlacesApiKey", "openaiApiKey", "customApiKey", "rapidApiKey", "cloudinaryApiSecret", "supabaseKey", "metaAccessToken", "metaVerifyToken", "telegramBotToken"] as const;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        ...
        <div>
          <label className={labelClass}>Callback URL (pasang ini di RapidAPI/gateway lo)</label>
          <div className="flex items-center gap-1.5">
            <code className="flex-1 truncate rounded-lg border border-neutral-200 bg-neutral-50 px-2.5 py-1.5 text-[11.5px] text-neutral-600 dark:border-neutral-800 dark:bg-neutral-900 dark:text-neutral-300">
              {webhookId ? `https://<domain-lo>/api/webhooks/gateway/${webhookId}` : "Memuat..."}
            </code>
            {webhookId && (
              <button
                type="button"
                onClick={() => {
                  navigator.clipboard.writeText(`https://<domain-lo>/api/webhooks/gateway/${webhookId}`);
                  setWebhookIdCopied(true);
                  setTimeout(() => setWebhookIdCopied(false), 2000);
                }}
                className="shrink-0 rounded-md bg-neutral-900 px-2.5 py-1.5 text-[11px] font-medium text-white dark:bg-neutral-100 dark:text-neutral-900"
              >
                {webhookIdCopied ? "Tersalin!" : "Salin"}
              </button>
            )}
          </div>
          <p className="mt-1 text-[10.5px] leading-relaxed text-neutral-400">
            URL ini unik buat akun lo — ganti &lt;domain-lo&gt; dengan domain Zales lo. Jangan share URL ini, siapapun
            yang tau bisa memicu workflow WA Gateway lo.
          </p>
        </div>
