...
/**
 * Returns this user's WA Gateway webhook ID, generating and persisting one
 * on first call if they don't have one yet. This ID is what makes
 * /api/webhooks/gateway/<id> unique per user — without it, every account's
 * generic WA Gateway workflows would share one indistinguishable inbox (see
 * the pre-fix version of that route for why that's a real cross-tenant bug,
 * not just a theoretical one).
 */
export async function getOrCreateWebhookId(userId: string): Promise<string> {
  const raw = await getRawSettings(userId);
  if (raw.webhookId) return raw.webhookId;

  const webhookId = crypto.randomUUID().replace(/-/g, "");
  const merged: UserSettings = { ...raw, webhookId };
  await query(`update users set settings = $2::jsonb where id = $1`, [userId, JSON.stringify(merged)]);
  return webhookId;
}

/** Looks up which user owns a given WA Gateway webhook ID. Used by the
 *  webhook route to route an incoming message to the right (and only the
 *  right) user's workflows. */
export async function findUserIdByWebhookId(webhookId: string): Promise<string | null> {
  const rows = await query<{ id: string }>(
    `select id from users where settings->>'webhookId' = $1`,
    [webhookId]
  );
