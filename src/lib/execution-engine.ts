...
/** WA/Telegram triggers describe attachments with a coarse mediaType string
 *  ("image" | "video" | "audio" | "document") rather than a precise MIME
 *  type. Used as a fallback when the fetched file's own Content-Type header
 *  is missing or generic (e.g. application/octet-stream). */
function guessMimeTypeFromMediaType(mediaType: string | undefined): string {
  switch (mediaType) {
    case "image":
      return "image/jpeg";
    case "video":
  ...
  switch (kind) {
    ...
    case "trigger.telegram":
      return {
        triggered: true,
        at: new Date().toISOString(),
        botToken: triggerData?.botToken ?? null,
    ...
    case "ai.vision": {
      ...
      const mediaParts: { inlineData: { mimeType: string; data: string } }[] = inputFiles
        .filter((f) => f.dataBase64 && f.mimeType)
        .map((f) => ({ inlineData: { mimeType: f.mimeType as string, data: f.dataBase64 as string } }));

      // Fallback for URL-based channels (WhatsApp, Telegram) that don't hand
      // over raw base64 the way the Chat Box trigger does — fetch the file
      // ourselves and inline it. Without this, voice notes/photos coming
      // from Telegram or WhatsApp would silently have nothing to analyze.
      if (mediaParts.length === 0) {
        const mediaUrl = (input as { mediaUrl?: unknown } | undefined)?.mediaUrl;
        if (typeof mediaUrl === "string" && mediaUrl) {
          try {
            const mediaRes = await fetchWithTimeout(mediaUrl, {}, 60_000);
            if (mediaRes.ok) {
              const buf = Buffer.from(await mediaRes.arrayBuffer());
              const mimeType =
                mediaRes.headers.get("content-type") ||
                guessMimeTypeFromMediaType((input as { mediaType?: unknown } | undefined)?.mediaType as string | undefined);
              mediaParts.push({ inlineData: { mimeType, data: buf.toString("base64") } });
            }
          } catch {
            // best-effort — falls through to the "no media" error below if this was the only source
          }
        }
      }
    ...
    case "integration.telegram": {
      const botToken = (params.botToken as string) || accountSettings.telegramBotToken || "";
      ...
      if (!botToken) throw new Error("Telegram node needs a Bot Token — isi langsung di node, atau simpan sekali di Pengaturan Akun > API Keys.");
    ...
    case "integration.telegram_reply": {
      // Priority: explicit override on this node > the exact bot token that
      // received this message (from the trigger) > the account-wide
      // default. Falling straight to the account-wide token would send
      // replies from the wrong bot for anyone running more than one.
      const triggerBotToken = (triggerData?.botToken as string) || ((input as { botToken?: string } | undefined)?.botToken) || "";
      const botToken = (params.botToken as string) || triggerBotToken || accountSettings.telegramBotToken || "";
      const message = resolveTemplate((params.message as string) || "", input, nodesByLabel);
      // Chat ID is auto-detected from the ORIGINAL trigger data (not the
      // immediate input), so this still works even when other nodes (like
      // an AI Agent) sit between the Telegram trigger and this reply node
      // and don't themselves pass chatId through in their own output.
      const chatId = triggerData?.chatId ?? (input as { chatId?: unknown } | undefined)?.chatId;

      if (!botToken) {
        throw new Error(
          "Balas ke Telegram node needs a Bot Token — isi langsung di node, atau simpan sekali di Pengaturan Akun > API Keys."
        );
      }
      if (!chatId) {
        throw new Error(
          "Tidak ada Chat ID di trigger — node ini cuma bisa dipakai setelah trigger 'Telegram Bot' (langsung atau lewat node lain di antaranya)."
        );
      }
      if (!message.trim()) {
        throw new Error("Balas ke Telegram node's Pesan Balasan kosong setelah template di-resolve.");
      }
      ...
      return { reply: message, sent: true, messageId: json?.result?.message_id, chatId };
    }
